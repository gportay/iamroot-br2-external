...
define SKELETON_PACSTRAP_BUILD_CMDS
	...
	if [ "$(SKELETON_PACSTRAP_KEYRINGS)" ]; then \
		PATH=$(BR_PATH) ido --libdir pacman-key --populate $(SKELETON_PACSTRAP_KEYRINGS) --gpgdir $(O)/gnupg; \
	fi
	( cd $(@D) && $(TARGET_MAKE_ENV) $(SKELETON_PACSTRAP_ENV) ido --libdir pacstrap -GMC $(HOST_DIR)/etc/pacman.conf $(@D)/rootfs $(SKELETON_PACSTRAP_PACKAGES) --arch $(ARCH) --dbpath $(@D)/rootfs/var/lib/pacman --gpgdir $(O)/gnupg )
endef
