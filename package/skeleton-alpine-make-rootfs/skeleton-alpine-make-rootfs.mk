...
SKELETON_ALPINE_MAKE_ROOTFS_ENV += IDO_SECURE_PATH=$(HOST_DIR)/bin:/usr/local/bin:/usr/bin:/bin
...
define SKELETON_ALPINE_MAKE_ROOTFS_BUILD_CMDS
	...
	( cd $(@D) && PATH=$(BR_PATH) $(SKELETON_ALPINE_MAKE_ROOTFS_ENV) APK_OPTS="--arch $(SKELETON_ALPINE_MAKE_ROOTFS_ARCH)" ido --preserve-env=APK_OPTS alpine-make-rootfs --packages "$(SKELETON_ALPINE_MAKE_ROOTFS_PACKAGES)" --repositories-file $(@D)/repositories --keys-dir $(HOST_DIR)/etc/apk/keys $(@D)/rootfs )
endef
